...
//
// It is priority queue, depth first taking the closet
// key each time
//
// Data structures are rather inefficient and there is a lot of copying
...
const queue: [
  ...
  Generator<[Position, string, number]>,
  string[],
][] = [[0, 0, grid.find("@"), gc, nearestKeys(gc, grid.find("@")), []]];
...
do {
  ...
  const [k, d, p, g, n, keys] = queue.shift();
  ...
  queue.push([k + 1, cd, np, ng, nearestKeys(ng, np), [...keys, v]]);
} while (queue.length > 0);
