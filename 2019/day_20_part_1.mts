...
// This is just a BFS using a priority queue on the shortest path (i.e. Dijkstra)
// The main issue is parsing the input to generate the portals
...
class PositionMap {
  #grid = new Map<number, Map<number, Position>>();

  get([x, y]: Position): Position | undefined {
    return this.#grid.get(y)?.get(x);
  }

  set(from: Position, to: Position) {
    const [x1, y1] = from;
    if (!this.#grid.has(y1)) {
      this.#grid.set(y1, new Map());
    }
    this.#grid.get(y1).set(x1, to);
  }
...
const portalList = new Map<string, [Position, Position][]>();
...
portalList.forEach(([[en1, ex1], [en2, ex2]]) => {
  portals.set(en1, ex2);
  portals.set(en2, ex1);
});

const visited = new Grid<number>([[start, 0]]);
const queue: [number, Position][] = [[0, start]];
