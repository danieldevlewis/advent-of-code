...
let oxygen: [number, number];

function coord(path: number[]): [number, number] {
...
do {
  ...
  //
  // When we find the oxygen source, reset the grid and paths
  // and redraw from that location
  //
  // The time is the longest recorded path
  //
  ...
  if (result === 2) {
    // Reset the grid and start the walk again
    draw(grid);
    grid.clear();
    grid.set(0, new Map([[0, 0]]));
    paths.splice(0, paths.length);
    paths.push([NORTH], [SOUTH], [EAST], [WEST]);
    continue;
  }
  if (result === 1) {
...
console.log(
  Math.max(
    ...grid
      .values()
      .map((r) => Math.max(...r.values().filter((v) => v < Infinity))),
  ),
);
