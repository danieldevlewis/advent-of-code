...
// This one is just logical deduction
//
// Jump as soon as a hole is 3 away (or 1 or 2, if just jumped)
// And there is landing 4 away, and either
// a landing 8 away (second jump) or a solid
// platform to walk to 7 away
...
const input = instructions.flatMap((l) => [
  ...l.split("").map((c) => c.charCodeAt(0)),
  10,
]);

for (const d of run(input)) {
  if (d > 255) {
